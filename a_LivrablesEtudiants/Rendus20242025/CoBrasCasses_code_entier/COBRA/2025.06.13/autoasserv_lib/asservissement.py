...
def asservir_tout(MyPIDs, last_measures,consigne_position,matrice_etats):
    ...
    print("erreurs:",erreur_R_dir[config.HEADING_INDEX])

    efforts = np.array([0 for i in range(config.NB_AXES)])
    efforts[config.Z_INDEX] = 2#MyPIDs[config.Z_INDEX].compute(erreur_R_dir[config.Z_INDEX])
    efforts[config.X_INDEX] = 1#MyPIDs[config.X_INDEX].compute(erreur_R_dir[config.X_INDEX])
    efforts[config.HEADING_INDEX] = MyPIDs[config.HEADING_INDEX].compute(erreur_R_dir[config.HEADING_INDEX])
    efforts_sat = np.array([min(max(float(efforts[i]),config.Saturation_Efforts[i]["min"]),config.Saturation_Efforts[i]["max"]) for i in range(config.NB_AXES)])
...
def update_etat_loop(autorise_asservissement, MyMergeMeas, matrice_etats):
    ...
    i=0
    MyPIDs = [None for i in range(config.NB_AXES)]
    MyPIDs[config.Z_INDEX] = PID.PID(P=60,ITs=0.184,D=120,N=4.07,Ts=0.112)
    MyPIDs[config.HEADING_INDEX] = PID.PID(P=0.1,ITs=3.41e-6,D=80,N=5,Ts=0.112)#P=1,ITs=1.84e0,D=20,N=4.07,Ts=0.112)  (P=0.1,ITs=3.41e-6,D=80,N=10,Ts=0.112)
    while True:
        ...
        if autorise_asservissement==True:
            asservissements_dict = asservir_tout(MyPIDs,measures_array,config.Consignes_Positions[:,i],matrice_etats)
        else:
