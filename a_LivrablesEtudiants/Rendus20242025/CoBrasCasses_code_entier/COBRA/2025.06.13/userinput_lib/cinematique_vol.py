...
L=0.01 #en m
a=1.6/10 #N/pourcent de commande moteur
h=0.01 #en m

def cmd_forces(liste_forces_couples):
    consignes_moteurs = cinematique_vol_ter(liste_forces_couples)
    print("consignes_moteurs:",consignes_moteurs)
...
def cinematique_vol_ter(liste_forces_couples):
    ...
    N_F=(fx**2+fy**2+fz**2)**(1/2)
    commandes = [0 for i in range(config.NB_ACTIONNEURS)]
    commandes[config.GAUCHE_INDEX]  = np.sign(fz)/(2*a)*N_F*(1-h/L*Cz/Cy)
    commandes[config.DROITE_INDEX]  = np.sign(fz)/(2*a)*N_F*(1+h/L*Cz/Cy)
    commandes[config.AXE_INDEX]     = theta*180/np.pi
    commandes[config.CERCEAU_INDEX] = np.sign(fz)*np.arcsin(fy/N_F)*180/np.pi
    return commandes
