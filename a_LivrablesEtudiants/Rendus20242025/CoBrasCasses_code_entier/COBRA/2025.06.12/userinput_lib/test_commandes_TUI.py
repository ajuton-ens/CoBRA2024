...
forces_list[cine_vol.CX_INDEX] = ["Roulis","useless_column"]
forces_list[cine_vol.CY_INDEX] = ["Tangage","useless_column"]
forces_list[cine_vol.CZ_INDEX] = ["Lacet","useless_column"]
...
def set_motor_value(motor, page_current, val, motor_index):
    match page_current:
        ...
        case page if page==PAGE_FORCES:
            asserv.autorise_asservissement = False
            cine_vol.cmd_force(motor_index, val)
    ...
    while key != ord('q'):
        match key:
            ...
            case curses.KEY_ENTER | 10 | 13:
                motor = pages[page_current][cursor_y][1]
                set_motor_value(motor,page_current,float(input_nb),cursor_y)
                input_nb=""
