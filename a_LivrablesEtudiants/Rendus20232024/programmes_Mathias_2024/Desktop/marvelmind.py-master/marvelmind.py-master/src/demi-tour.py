...
while True :
    ...
    try:
        ...
        yaw_filt.append((angles_filt[2]+360)%360 - yaw_initialisation)
        ...
        if stateA:
            ...
            #     if yaw_filt[-1] < -1 + yaw_offset:
            #         stateA_A = False
            #         stateA_B = True
            #         stateA_C = False
            #         time_yaw_init = time_current

            #     elif yaw_filt[-1] > 1 + yaw_offset:
            #         stateA_A = False
            #         stateA_B = False
            #         stateA_C = True
            #         time_yaw_init = time_current
            ...
            # elif stateA_B:
            #     if time_current - time_yaw_init < 1:
            #         back_motor.set_speed(ecart_yaw/50)
            #     else:
            #         stateA_A = True
            #         stateA_B = False
            #         stateA_C = False
        ...
        if stateB:
             """ Demi tour """
             back_motor.set_speed(0.25)
             main_motor.set_speed(0)

            # Arrête le moteur arrière et change la consigne            
             if (yaw_filt[-1]+360)%360 < 160 :
                PID_yaw = PID(1/80,0,1/150,270)
                stateA = False
                stateB = False
                stateC = True
