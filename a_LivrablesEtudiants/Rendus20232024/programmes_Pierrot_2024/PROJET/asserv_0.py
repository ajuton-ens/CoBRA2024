...
mcc_yaw = MCC(MOTOR_YAW, 2)
mot_alt = brushless(BRUSHLESS, 1)
mot_prop = brushless(BRUSHLESS, 0)
capt = capteurs()
alt_obj = 170
#boucle_continue
"""
while True : 
    try : 
        ...
        mot_angle.commande_angle(90 + pitch)
        alt_mes = distance_us*np.cos(roll)*np.cos(pitch)*np.cos(yaw)
        eps = alt_obj - alt_mes
        mot_brushless.commande_puissance((np.tanh(eps/10)+1)/2)
