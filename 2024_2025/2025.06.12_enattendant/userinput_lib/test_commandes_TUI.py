...
class TUI:
    def __init__(self, stdscr, matrice_etats, autorise_asservissement):
        ...
        self.sensor_data_list   = np.array([["aa",0] for i in range(len(config.NB_AXES))])
        ...
        self.forces_list = np.array([["aa",0] for i in range(len(config.NB_AXES))])
    ...
    def set_motor_value(self, page_i, motor_i, val):
        ...
        match page_i:
            ...
            case page if page==PAGE_SENSORS:
                ...
                self.consigne_position[motor_i] = val
            case page if page==PAGE_FORCES:
                self.autorise_asservissement = False
                measures_array = self.matrice_etats[config.MESURES_INDEX*config.NB_AXES:config.MESURES_INDEX*(config.NB_AXES+1),-1].copy()
                asservissements_dict = asserv.create_null_asserv_dict()
                asservissements_dict["COMMANDES_FORCES"] = self.matrice_etats[config.COMMANDES_FORCES_INDEX*config.NB_AXES:config.COMMANDES_FORCES_INDEX*(config.NB_AXES+1),-1].copy()
    ...
    def refresh_TUI(self):
        ...
        for page_i in range(0,len(self.pages)):
            for motor_i in range(0, len(self.pages[page_i])):
                ...
                self.stdscr.addstr(motor_i, PAGE_WIDTH*page_i+10, str(round(self.get_motor_value(page_i,motor_i),5)))
        self.stdscr.addstr(height-2, 10, str(input_nb))
        self.stdscr.move(cursor_y,PAGE_WIDTH*page_current)
        self.stdscr.refresh()
