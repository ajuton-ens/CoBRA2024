...
def asservir_commande(erreur_R_dir,matrice_etats):
    d_erreur_dt = matrice_etats[config.ERREURS_INDEX*config.NB_AXES:config.ERREURS_INDEX*(config.NB_AXES+1),-1]-matrice_etats[config.ERREURS_INDEX*config.NB_AXES:config.ERREURS_INDEX*(config.NB_AXES+1),-3]
    d_erreur_dt2 = matrice_etats[config.ERREURS_INDEX*config.NB_AXES:config.ERREURS_INDEX*(config.NB_AXES+1),-3]-matrice_etats[config.ERREURS_INDEX*config.NB_AXES:config.ERREURS_INDEX*(config.NB_AXES+1),-6]
    acceleration = d_erreur_dt - d_erreur_dt2

    return erreur_R_dir*config.Kp*(1+matrice_etats[config.ERREURS_INDEX*config.NB_AXES:config.ERREURS_INDEX*(config.NB_AXES+1),:].sum(axis=1)/config.Ti-config.Td*d_erreur_dt)


def asservir_tout(last_measures,consigne_position,matrice_etats):
    asservissements_dict = {}
    asservissements_dict["CONSIGNES_POSITION"] = np.zeros((config.NB_AXES))
    ...
    asservissements_dict["CONSIGNES_POS"] = cons
...
def update_matrice_etats(matrice_etats, current_time, measures_array, asservissements_dict):
    ...
    matrice_etats[config.MESURES_INDEX*config.NB_AXES:config.MESURES_INDEX*(config.NB_AXES+1),-1]                         = measures_array.copy()
    matrice_etats[config.CONSIGNES_POSITIONS_INDEX*config.NB_AXES:config.CONSIGNES_POSITIONS_INDEX*(config.NB_AXES+1),-1] = asservissements_dict["CONSIGNES_POSITIONS"].copy()
    matrice_etats[config.ERREURS_INDEX*config.NB_AXES:config.ERREURS_INDEX*(config.NB_AXES+1),-1]                         = asservissements_dict["ERREURS"].copy()
    matrice_etats[config.COMMANDES_FORCES_INDEX*config.NB_AXES:config.COMMANDES_FORCES_INDEX*(config.NB_AXES+1),-1]       = asservissements_dict["COMMANDES_FORCES"].copy()

def create_null_asserv_dict():
    none_array = np.array([None for i in range(config.NB_ETATS)])
...
def update_etat_loop(autorise_asservissement, MyMergeMeas, matrice_etats):
    ...
    while True:
        ...
        if autorise_asservissement==True:
            asservissements_dict = asservir_tout(measures_array,config.Consignes_Positions[:,i],matrice_etats)
        else:
